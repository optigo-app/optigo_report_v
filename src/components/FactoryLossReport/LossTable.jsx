    ...
    <>
      ...
      <ResponsiveContainer width="100%" height="95%">
        <PieChart>
          ...
          <Pie data={formattedData} dataKey="count" nameKey="range" cx="50%" cy="50%" outerRadius={125} innerRadius={65} cornerRadius={6} paddingAngle={4} labelLine={false} 
            // label={({ name, value }) => `${name} (${value})`} //09/12/25_10:10 
            onClick={(data, index, e) => handleSliceClick(e, data.payload.details)}
          >
            {formattedData?.map((entry, index) => (
              <Cell  fill={`url(#gradient-${index})`} stroke="#fff" strokeWidth={0} cursor="pointer" />
            ))}
          </Pie>
